...
class Debian(base.Base):
    def __init__(self, image, packages, machines, apt_sources_update=[]):
        name, version = image.split(':')
        version = version.split('-')[0]
        self._packages = packages
        super().__init__(image, name, version, package_manager.APT(run_once=apt_sources_update), machines=machines)
...
DEBIAN_LIST = [
    ...
    Debian('debian:7-slim', machines=['i686', 'x86_64', 'armv7l'], packages=[DEBIAN_PACKAGES + ['curl']], apt_sources_update=DEBIAN_APT_OLD),
]
